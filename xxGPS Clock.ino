/*
  GPS -> UTC offset for a NodeMCU (ESP8266) + NEO-6M clock project.

  Flow:
    1. Read NMEA sentences from the NEO-6M over SoftwareSerial (the ESP8266's
       one hardware UART is tied up with USB / Serial Monitor, so GPS needs
       its own software-emulated serial port on spare GPIOs).
    2. Parse lat/lon and UTC time/date out of a GPRMC/GNRMC sentence with TinyGPS++.
    3. Once we have a fix, connect to WiFi and query a timezone API with the
       lat/lon to get back the UTC offset (in seconds) and whether DST is
       currently in effect.
    4. Cache the offset and only re-query occasionally (e.g. once a day, or on
       boot) — you don't need to hit the API every loop.

  Libraries needed (Library Manager):
    - TinyGPSPlus (by Mikal Hart)
    - ArduinoJson (by Benoit Blanchon)
    - ESP8266WiFi.h / ESP8266HTTPClient.h / SoftwareSerial.h are built into
      the ESP8266 Arduino core (make sure "esp8266" boards package is installed,
      not just "esp32").

  Wiring (typical NodeMCU pin labels):
    NEO-6M TX  -> NodeMCU D2 (GPIO4)   [SoftwareSerial RX side]
    NEO-6M RX  -> NodeMCU D1 (GPIO5)   [SoftwareSerial TX side, optional]
    NEO-6M VCC -> 3.3V
    NEO-6M GND -> GND

  Timezone API used here: timeapi.io (free, no API key required as of writing).
  Swap buildTimezoneUrl()/parseTimezoneResponse() if you prefer Google Time
  Zone API or TimeZoneDB (both require a free API key).
*/
...
// including the delimiters. Rather than feeding TinyGPS++ one raw byte at a
// time as it arrives (which — combined with SoftwareSerial's own byte drops —
// made it hard to tell corruption in the *link* apart from corruption in our
// own handling), we frame sentences ourselves first: buffer until we see the
// '$'/'!' that starts one, keep buffering until the terminating '\n', then
// print and hand the whole clean sentence to gps.encode() in one go.
// A nice side effect: any stray/garbled bytes sitting between sentences (from
// a dropped bit, noise, etc.) are simply discarded rather than getting stuck
// to the front of the next sentence — the next '$' always resyncs us.
...
// Connects to WiFi (if not already), queries timeapi.io for the offset at
